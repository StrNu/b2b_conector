<?php
// Vista de registro de proveedores con diseño limpio y profesional
$pageTitle = 'Registro de Proveedores - ' . (isset($event) ? $event->getEventName() : 'Evento');
...
<div class="registration-container">
    ...
    <div class="registration-header">
        <h1 class="registration-title">Registro para Proveedores</h1>
        <p class="registration-subtitle">
            Complete el siguiente formulario para registrarse como proveedor en este evento. 
            Podrá especificar sus ofertas y productos disponibles.
        </p>
    </div>
    ...
    <form action="<?= BASE_URL ?>/suppliers_registration/<?= (int)$eventId ?>/store" method="POST" enctype="multipart/form-data" class="registration-form">
        ...
        <section class="form-section">
            ...
            <div class="form-grid form-grid--2-cols">
                <div class="form-field">
                    ...
                    <input type="email" name="account_email" class="form-input" required 
                           placeholder="cuenta@ejemplo.com" value="<?= old('account_email') ?>">
                </div>
        ...
        <section class="form-section">
            ...
            <div id="assistants-list">
                <div class="form-grid form-grid--2-cols assistant-item">
                    <div class="form-field">
                        <label class="form-label form-label--required">Nombre</label>
                        <input type="text" name="assistant_first_name[]" class="form-input" required 
                               id="assistant_first_name_0" placeholder="Nombre" value="<?= old('assistant_first_name.0') ?>">
                    </div>
                    
                    <div class="form-field">
                        <label class="form-label form-label--required">Apellido</label>
                        <input type="text" name="assistant_last_name[]" class="form-input" required 
                               id="assistant_last_name_0" placeholder="Apellido" value="<?= old('assistant_last_name.0') ?>">
                    </div>
                    
                    <div class="form-field">
                        <label class="form-label form-label--required">Teléfono Celular</label>
                        <input type="tel" name="assistant_phone[]" class="form-input" required 
                               id="assistant_phone_0" placeholder="+52 222 123 4567" value="<?= old('assistant_phone.0') ?>">
                    </div>
                    
                    <div class="form-field">
                        <label class="form-label form-label--required">Correo Electrónico</label>
                        <input type="email" name="assistant_email[]" class="form-input" required 
                               id="assistant_email_0" placeholder="correo@ejemplo.com" value="<?= old('assistant_email.0') ?>">
                    </div>
                </div>
            </div>
        ...
        <!-- Sección: Ofertas de Proveedor -->
        <section class="form-section">
            <div class="form-section-header">
                <i class="fas fa-box form-section-icon"></i>
                <h2 class="form-section-title">Ofertas de Proveedor</h2>
            </div>
            
            <p class="form-help-text" style="margin-bottom: 1.5rem;">
                <strong>Seleccione al menos un producto o servicio que ofrece.</strong> Puede seleccionar varias opciones.
            </p>
            ...
            <?php if (!empty($activeCategories)): ?>
                <div class="tabs-container">
                    <div class="tabs-list" id="offer-tabs">
                        ...
                        <div class="tab-content" id="tab-<?= (int)$cat['event_category_id'] ?>" 
                            ...
                            <div class="checkbox-group">
                                <?php foreach ($activeSubcategories as $sub): ?>
                                    <div class="checkbox-item">
                                        <input type="checkbox" name="supplier_offers[]" 
                                               value="<?= (int)$sub['event_subcategory_id'] ?>" 
                                               id="offer_<?= $sub['event_subcategory_id'] ?>" class="checkbox-input offer-checkbox">
                                        <label for="offer_<?= $sub['event_subcategory_id'] ?>" class="checkbox-label">
        ...
        <div class="form-actions">
            <button type="submit" class="btn-primary">
                <i class="fas fa-paper-plane"></i> Registrarse como Proveedor
            </button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addAssistant = function() {
        ...
        div.innerHTML = `
            <div class="form-field">
                <label class="form-label form-label--required">Nombre</label>
                <input type="text" name="assistant_first_name[]" class="form-input" required placeholder="Nombre">
            </div>
            <div class="form-field">
                <label class="form-label form-label--required">Apellido</label>
                <input type="text" name="assistant_last_name[]" class="form-input" required placeholder="Apellido">
            </div>
            <div class="form-field">
                <label class="form-label form-label--required">Teléfono Celular</label>
                <input type="tel" name="assistant_phone[]" class="form-input" required placeholder="+52 222 123 4567">
            </div>
            <div class="form-field">
                <label class="form-label form-label--required">Correo Electrónico</label>
                <input type="email" name="assistant_email[]" class="form-input" required placeholder="correo@ejemplo.com">
            </div>
        `;
    ...
    window.showTab = function(tabId, button) {
        ...
        document.querySelectorAll('#offer-tabs .tab-button').forEach(btn => {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // Validar que se seleccione al menos una oferta
            const checkedOffers = document.querySelectorAll('.offer-checkbox:checked');
            if (checkedOffers.length === 0) {
                hasErrors = true;
                errorMessages.push('Debe seleccionar al menos un producto o servicio que ofrece.');
            }
...
function old($name, $default = '') {
    if (strpos($name, '.') !== false) {
        // Handle array notation like "assistant_first_name.0"
        $parts = explode('.', $name);
        $key = $parts[0];
        $index = $parts[1];
        return isset($_POST[$key][$index]) ? htmlspecialchars($_POST[$key][$index]) : $default;
    }
