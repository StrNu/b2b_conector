<?php
// Partial: Agenda for buyers with Material Design 3 - Table Layout
// Expects: $agendas (array of buyers with their agendas)
?>
<div class="agenda-table-layout">
    <?php 
    // Filter only buyers with appointments
    $buyersWithAppointments = array_filter($agendas, function($agenda) {
        return !empty($agenda['appointments']) && 
               isset($agenda['company']) && 
               ($agenda['company']['role'] ?? null) === 'buyer';
    });
    ?>
    <?php if (!empty($buyersWithAppointments)): ?>
        <div class="table-container">
            <table class="companies-agenda-table">
                ...
                <tbody>
                    <?php foreach ($buyersWithAppointments as $agenda): ?>
                        <?php
                        if (!isset($agenda['company']) || !is_array($agenda['company'])) continue;
                        $buyer = $agenda['company'];
                        ?>
                        <tr class="company-row company-row--buyer">
                            <td class="company-name-cell">
                                <div class="company-info">
                                    <i class="fas fa-shopping-cart company-icon"></i>
                                    <span class="company-name"><?= htmlspecialchars($buyer['company_name'] ?? '') ?></span>
                                </div>
                            </td>
                            ...
                            <td class="actions-cell">
                                <div class="action-buttons">
                                    <a href="<?= BASE_URL ?>/agendas/preview?event_id=<?= (int)($buyer['event_id'] ?? $event->getId() ?? 0) ?>&company_id=<?= (int)($buyer['company_id'] ?? 0) ?>" 
    ...
    <?php else: ?>
        <div class="no-agendas">
            ...
            <h3>No hay compradores con citas programadas</h3>
            <p>Los compradores aparecerán aquí una vez que tengan citas asignadas</p>
        </div>
    <?php endif; ?>
</div>

<style>
/* Material Design 3 Styles for Buyers Agenda Table Layout */
.agenda-table-layout {
...
.company-row--buyer {
    border-left: 3px solid var(--md-tertiary-40);
}
...
.btn-action--edit {
    background: var(--md-tertiary-container);
    color: var(--md-on-tertiary-container);
    border: 1px solid var(--md-tertiary-40);
}
