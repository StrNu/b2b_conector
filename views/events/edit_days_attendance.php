<div class="content max-w-lg mx-auto mt-8">
    ...
    <form action="<?= BASE_URL ?>/events/edit_days_attendance/<?= (int)$event->getId() ?>/<?= (int)$company->getId() ?>" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-2">
                <?php foreach ($eventDays as $day): ?>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="attendance_days[]" value="<?= htmlspecialchars($day) ?>" class="mr-2" <?= in_array($day, $selectedDays) ? 'checked' : '' ?>>
